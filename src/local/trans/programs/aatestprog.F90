...
REAL (KIND=JPRB),    ALLOCATABLE :: ZGPBUFL_BEFORE (:,:,:)
REAL (KIND=JPRB),    ALLOCATABLE :: ZNORM(:,:)
REAL (KIND=JPRB),    ALLOCATABLE :: ZNORM_MAX
REAL (KIND=JPRB),    ALLOCATABLE :: ZGPBUFG (:,:) 
REAL (KIND=JPRB),    ALLOCATABLE :: ZGPBUFG_BEFORE (:,:)
...
! Ajout THOMAS
ALLOCATE (ZGPBUFL_BEFORE (NGPTOT, IFLDGPG, 1))
ZGPBUFL_BEFORE = ZGPBUFL 
...
! ajout Thomas
  ALLOCATE (ZGPBUFG_BEFORE (NGPTOTG, 1))
  CALL GATH_GRID (PGPG=ZGPBUFG_BEFORE, KFGATHG=1_JPIM, KTO=[1_JPIM], PGP=ZGPBUFL_BEFORE(:,1:1,:))
  ! CALCUL DE NORME THOMAS
  !ZNORM = ABS((ZGPBUFL - ZGPBUFL_BEFORE)/(ZGPBUFL_BEFORE))
  ZNORM = ABS((ZGPBUFG - ZGPBUFG_BEFORE))
  ZNORM_MAX = MAXVAL(ZNORM)
  WRITE(*,*) 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
  WRITE(*,*) 'ZNORM_MAX =', ZNORM_MAX
  DEALLOCATE (ZGPBUFG)
  DEALLOCATE (ZGPBUFG_BEFORE)
ELSE
  CALL GATH_GRID (KFGATHG=1_JPIM, KTO=[1_JPIM], PGP=ZGPBUFL (:,1:1,:))
  CALL GATH_GRID (KFGATHG=1_JPIM, KTO=[1_JPIM], PGP=ZGPBUFL_BEFORE (:,1:1,:))
ENDIF
